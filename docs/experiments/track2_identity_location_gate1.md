...
> If the recurrent memory is corrected toward the true current slot while its
...
simulator target slot is used only to ask whether the observed representation
failure is causal. No trainable memory mechanism is authorized until this gate
passes.

## 0. Environment

Use the pinned `external/MIKASA-Robo` stack and the existing isolated
environment described in `scripts/README.md`. Download both official mu-VLA
checkpoints if they are not already present:
...
change action decoding, image preprocessing, memory-token count, or the
released checkpoints.

## 1. Horizon-control pilot: K=2 versus K=8

Before fitting any editor, rule out the simplest explanation: the K=2 policy
may fail because its training credit-assignment horizon is too short.

Tasks:

- `ShellGameShuffleTouch-VLA-v0` (primary, 60-step Short tracking task)
- `ShellGameShuffleTouch-Long-VLA-v0` (600-step Medium tracking replication)
...
for ckpt in k2 k8; do
  ...
  for task in ShellGameShuffleTouch-VLA-v0 ShellGameShuffleTouch-Long-VLA-v0; do
    PYTHONPATH=../../src uv run python ../../scripts/eval_mu_vla_identity_slot_intervention.py \
...
primary Short task, stop IPSI and reinterpret the failure as primarily a
TBPTT/temporal-credit problem. Otherwise continue. The Long task is a stress
replication, not a substitute for the primary stop rule.

## 2. Collect leakage-safe editor data

Fit the editor only on the primary Short task with the K=2 checkpoint. Use 100
episodes beginning at seed `4242424242`; these seeds are disjoint from the
horizon pilot and the later causal test. Retain every eighth recurrent memory
token (`0, 8, ..., 56`).
...
The collector stores simulator identity/slot labels only for offline
subspace fitting and causal diagnostics. They are never inputs to the normal
policy.
...
3. fits separate balanced ridge classifiers for hidden identity and current
   slot;
4. chooses ridge alpha from `{1,10,100,1000}` on 20 held-out dev episodes;
5. estimates the median natural latent update norm on train trajectories;
...
The IPSI direction for a wrong predicted slot `s_hat` and simulator target slot
`s*` is
...
`slot_only` control omits the identity projection. A `random_orthogonal`
control samples a direction in the same identity-orthogonal subspace with the
same edit norm.

## 4. Offline sanity before closed-loop GPU evaluation

Inspect `artifacts/reports/shell_shuffle_k2_ipsi_editor.json`. Do not proceed if
there are no post-swap dev rows, the identity classifier is degenerate, or the
chosen IPSI edit fails to improve the dev slot probe without preserving the
identity probe. This is a sanity check; it is not a paper result.
...
Edits are applied exactly once after each newly completed swap and before the
next VLA action.
...
Reuse the **same Short-task editor** on
`ShellGameShuffleTouch-Long-VLA-v0`; do not refit it on Long test episodes.
Run the same four conditions on 50 paired seeds starting at `4242524242`.
This asks whether the learned local memory geometry transfers across horizon
rather than merely fitting one task duration.

## 7. What is explicitly not authorized yet
...
memory that separates slowly changing identity content from fast dynamic state.
That method is a separate decision and must not be implemented before the
causal rescue is established.
