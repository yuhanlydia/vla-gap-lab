...
MED_DIR="$ROOT/artifacts/mikasa/icassp_state_motion_medium_n60"
FAST_DIR="$ROOT/artifacts/mikasa/icassp_state_motion_fast_n60"
REPORT_DIR="$ROOT/artifacts/reports/icassp_state_motion"
...
PYTHONPATH="$ROOT/src" uv run python "$ROOT/scripts/collect_mu_vla_state_motion_icassp.py" \
  ...
  --episodes 60 --start-seed 4243024242 --precision 4bit --resume \
  --output-dir "$MED_DIR"

PYTHONPATH="$ROOT/src" uv run python "$ROOT/scripts/collect_mu_vla_state_motion_icassp.py" \
  --checkpoint "$CKPT" --task InterceptFast-VLA-v0 \
  --episodes 60 --start-seed 4243124242 --precision 4bit --resume \
  --output-dir "$FAST_DIR"
...
PYTHONPATH="$ROOT/src" "$MIKASA/.venv/bin/python" "$ROOT/scripts/summarize_icassp_state_motion.py" \
  ...
  --tracked-summary "$ROOT/results/track2_icassp_state_motion_summary.json"

echo "ICASSP study complete. Commit and push results/track2_icassp_state_motion_summary.json for review."
