...
ALLOWED_TASKS = ("InterceptMedium-VLA-v0", "InterceptFast-VLA-v0")
...
def full_memory(memory) -> np.ndarray:
    tokens = memory.float()[0]
    if tokens.ndim != 2 or tokens.shape[0] != 64:
        raise RuntimeError(f"expected recurrent memory [64, hidden], got {tuple(tokens.shape)}")
    return tokens.detach().cpu().numpy().astype(np.float16, copy=False)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--task", choices=ALLOWED_TASKS, required=True)
    parser.add_argument("--episodes", type=int, default=60)
    parser.add_argument("--start-seed", type=int, required=True)
    parser.add_argument("--precision", choices=["4bit"], default="4bit")
    ...
    base = env.unwrapped
    required = ("ball", "goal_region", "agent", "reached_status")
    missing = [name for name in required if not hasattr(base, name)]
    if missing:
        raise ValueError(f"task does not expose ICASSP state-motion labels: {missing}")
    ...
    try:
        for episode_index in range(args.episodes):
            ...
            if path.exists():
                if not args.resume:
                    raise ValueError(f"output exists; pass --resume to reuse: {path}")
                _, metadata = load_episode_npz(path)
                for key, value in expected.items():
                    if metadata.get(key) != value:
                        raise ValueError(f"{path}: resume metadata mismatch for {key}")
                continue
            ...
            arrays = {
                ...
                "goal_position_xy": np.asarray(rows["goal_position_xy"], dtype=np.float32),
                "tcp_position_xy": np.asarray(rows["tcp_position_xy"], dtype=np.float32),
            ...
            save_episode_npz_atomic(
                path,
                arrays,
                {**expected, "steps": len(rows["step"]), "success_once": success_once},
            )
            print(json.dumps({
                "episode": episode_index,
                "seed": seed,
